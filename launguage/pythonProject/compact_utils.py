...
from pythonProject.utils import replace, isConvertString
import re
...
class MultiLanguageComparison:
    ...
    # parser：解析器
    @staticmethod
    def compact(fileDirs, source, compact, target, parser):
        ...
        for k in doc1.keys():
            if k not in doc2:
                ...
                # 如果K在doc2 也在doc1的情况下
                if doc2[k] is not None and doc1[k] is not None:
                     doc2[k] = doc1[k]
        ...
        for k in doc1.keys():
            if k in doc2:
                doc2_sorted_dict[k] = doc1[k]
            else:
                doc2_sorted_dict[k] = doc2.get(k, '')
        for v in doc2.keys():
            doc2_sorted_dict[v] = doc2[v]
        # 还原字典KEY
        recoveryDict = {}
        for v in finalDict.keys():
            ...
            if result in doc2_sorted_dict:
                ...
                recoveryDict[v] = ''
        parser.parser(recoveryDict, os.path.join(fileDirs, target))

    @staticmethod
    def compact2(fileDirs, source, compact, target, parser):
        tree = ET.parse(os.path.join(fileDirs, source))
        root = tree.getroot()
        # 生成字典
        doc1 = {}
